...
def calculate_targets_at(anchor_center, txt_list, anchor_heights):
    ...
    for item in txt_list:
        ...
        if (bbox[0] < wc and wc <= bbox[2]):
            flag = 1
        elif (wc < bbox[0] and bbox[2] < wc+asw):   ## find the anchor is near to left or right side of GT.
    ...
    for idx, ah in enumerate(anchor_heights):
        ...
        ratio_bbox = [0, 0, 0, 0]
        #
        ratio = (text_bbox[0]-anchor_bbox[0]) /anchor_width ## offset_X_start=(Xside-anchor_cneter)/anchor_width
        if abs(ratio) < 1: 
            ratio_bbox[0] = ratio
        #
        # print(ratio)
        #
        ratio = (text_bbox[2]-anchor_bbox[2]) /anchor_width ## offset_X_end
        if abs(ratio) < 1:
            ratio_bbox[2] = ratio
        #
        # print(ratio)
...
def get_image_and_targets(img_file, txt_file, anchor_heights):
    ...
    # ///2, ///2, //3, -2
    # ///2, ///2, ///2,    
    # for size of pooling & padding --> feature map
    ...
    target_ver = np.zeros((height_feat, width_feat, 2*num_anchors)) ## for Xs,Xe
    target_hor = np.zeros((height_feat, width_feat, 2*num_anchors)) ## for Ys,Ye
    #
    ...
    for h in range(height_feat):
        ...
        for w in range(width_feat):
            #
            cls,ver,hor = calculate_targets_at([hc, wc_start + asw * w], txt_list, anchor_heights) ## txt_list:一个图片中的真值bbox，该函数细化成小框
            #
            target_cls[h, w] = cls
            target_ver[h, w] = ver
            target_hor[h, w] = hor
...
def trans_results(r_cls, r_ver, r_hor, anchor_heights, threshold):
    ...
    for h in range(feat_shape[0]):
        ...
        for w in range(feat_shape[1]):
            ...
            #
            #r_cls 产生了8个不同height对应的confidence， //2 将其映射到4个，对应于anchor_heights[4]=<class 'list'>: [6, 12, 24, 36]
            anchor_posi = np.argmax(r_cls[h,w,:])  # in r_cls
            anchor_id = anchor_posi //2  # in anchor_heights, id range from 1 to 4.
            #
            #print(anchor_id)
            #print(r_cls[h,w,:])
            #
...
def get_successions(index, text_proposals, boxes_table, im_size, heights):
    box = text_proposals[index]
    results = []
    # for left in range(int(box[0]) + 1, min(int(box[0]) + cfg.MAX_HORIZONTAL_GAP + 1, im_size[1])): #### min(int(box[0]) + cfg.MAX_HORIZONTAL_GAP + 1, im_size[1]):判断是否超过x右边界
    for left in range(int(box[0]) + 1, min(int(box[0]) + cfg.MAX_HORIZONTAL_GAP + 1, 800)):
        adj_box_indices = boxes_table[left]
        for adj_box_index in adj_box_indices:
            if meet_v_iou(adj_box_index, index, text_proposals, heights):
                results.append(adj_box_index)
...
def get_text_lines2(text_proposals, scores, im_size):
    """
    text_proposals:boxes

    """
    # tp=text proposal
    tp_groups = group_text_proposals(text_proposals, scores, im_size)  # 首先还是建图，获取到文本行由哪几个小框构成
